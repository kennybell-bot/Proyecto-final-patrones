...
import { Tabs, Tab } from './tabs'; // Ruta correcta
  ...
  useEffect(() => {
    const fetchStoreStats = async () => {
        try {
            ...
            // Convertir sold y price a números y ordenar por vendidos
            const formattedData = data.map(product => ({
                ...product,
                sold: parseInt(product.sold, 10),
                price: parseFloat(product.price.replace("$", ""))
            })).sort((a, b) => b.sold - a.sold);

            setStoreStats(formattedData);
        } catch (error) {
